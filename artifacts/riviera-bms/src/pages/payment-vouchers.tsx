...
import { useListPaymentVouchers, useCreatePaymentVoucher, useGetExchangeRates } from "@workspace/api-client-react";
...
const emptyForm = { date: new Date().toISOString().split("T")[0], beneficiaryName: "", amount: "", currency: "ILS", exchangeRate: "1", paymentMethod: "cash", category: "", chequeNumber: "", bankName: "", chequeDate: "", dueDate: "", notes: "" };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold">سندات الصرف</h1><p className="text-muted-foreground mt-1">تسجيل المصروفات والمدفوعات</p></div>
        <Button onClick={() => { setForm({ ...emptyForm }); setOpen(true); }} className="flex items-center gap-2"><Plus size={16} />إصدار سند صرف</Button>
      </div>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              ...
              {vouchers.length === 0 && <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">لا توجد سندات</TableCell></TableRow>}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg" dir="rtl">
          <DialogHeader><DialogTitle>إصدار سند صرف جديد</DialogTitle></DialogHeader>
          <div className="grid gap-3 py-2 max-h-[60vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label>المستفيد *</Label><Input value={form.beneficiaryName} onChange={e => setForm(f => ({ ...f, beneficiaryName: e.target.value }))} className="mt-1" /></div>
              <div>
          ...
          <DialogFooter className="flex-row-reverse gap-2">
            <Button onClick={handleSave} disabled={create.isPending}>إصدار</Button>
