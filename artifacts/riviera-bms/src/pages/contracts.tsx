...
export default function Contracts() {
  ...
  const openEdit = (c: any) => {
    setEditing(c.id);
    setForm({ tenantId: String(c.tenantId), unitId: String(c.unitId), startDate: c.startDate, endDate: c.endDate, rentAmount: String(c.rentAmount), currency: c.currency, exchangeRate: String(c.exchangeRate), paymentFrequency: c.paymentFrequency, notes: c.notes ?? "" });
    setOpen(true);
  };

  const handleSave = async () => {
    const payload = { ...form, tenantId: Number(form.tenantId), unitId: Number(form.unitId), rentAmount: Number(form.rentAmount), exchangeRate: Number(form.exchangeRate), rentAmountILS: amountILS };
    try {
      if (editing) {
                  ...
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        <Button size="sm" variant="ghost" className="text-destructive" onClick={() => handleDelete(c.id)}><Trash2 size={14} /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                );
              })}
              {contracts.length === 0 && <TableRow><TableCell colSpan={8} className="text-center py-8 text-muted-foreground">لا توجد عقود</TableCell></TableRow>}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
          ...
          <div className="grid gap-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select value={form.unitId} onValueChange={v => setForm(f => ({ ...f, unitId: v }))}>
                  ...
                  <SelectContent>{(units as any[]).filter((u: any) => u.status !== "occupied" || String(u.id) === form.unitId).map((u: any) => <SelectItem key={u.id} value={String(u.id)}>وحدة {u.unitNumber}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select value={form.paymentFrequency} onValueChange={v => setForm(f => ({ ...f, paymentFrequency: v }))}>
                <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="monthly">شهري</SelectItem><SelectItem value="quarterly">ربع سنوي</SelectItem><SelectItem value="annually">سنوي</SelectItem></SelectContent>
              </Select>
            </div>
